...
Unbiasly AI is a startup-grade, real-time, explainable AI bias detection engine. It combines a local deep-learning pipeline (**BART Zero-Shot Classifier**) for objective multi-label bias scoring, **LIME (Local Interpretable Model-agnostic Explanations)** for token-level visual attention heatmaps in sequential order, and a large language model (**Google Gemini**) for advanced semantic classification and contextual rewrites.
...
*   **Key Design Patterns**:
    *   **Volume Isolation**: Maps `./backend/app/models/zero_shot` from host to container, decoupling heavy model weights (1.6GB+) from container builds.
...
*   **Key Design Patterns**:
    *   **Non-Blocking Event Loop (ThreadPoolExecutor)**: BART LIME token perturbations are CPU-intensive and synchronous. To prevent blocking the main FastAPI event loop, these operations are offloaded onto a background threadpool.
...
*   **Role**: Handles GPU/CPU execution of the `facebook/bart-large-mnli` zero-shot classifier and LIME explainability.
*   **Key Design Patterns**:
    *   **Singleton Model Manager**: Instantiates `ModelManager` as a singleton to share the PyTorch GPU/CPU zero-shot pipeline.
    *   **Graceful Degraded Heuristics**: If model initialization fails, it falls back gracefully to prevent startup crashes.
...
graph LR
    ...
    subgraph Host ["HOST OS SYSTEM"]
        subgraph Directory_Map ["Directory Map"]
            Weights["./backend/app/models/zero_shot"]
            Env[".env File"]
    ...
    subgraph Docker_Network ["Docker Compose Bridge Network"]
        ...
        subgraph Backend_Container ["unbiasly-backend Container (FastAPI:slim)"]
            Container_Model["/app/app/models/zero_shot"]
...
sequenceDiagram
    ...
    participant BART as PyTorch (BART Zero-Shot)
    ...
    rect rgb(30, 41, 59)
        ...
        CPU->>BART: Load text tensor & classify
        BART-->>CPU: Class probabilities & token weights
    ...
    Note over User: DOM gauges render and words colorize in sequence!
```
