...
classDiagram
    ...
    ModelManager --> BERT_LoRA_Weights : Loads weights on boot (Singleton)
...
*   **`ModelManager`**: Standardizes PyTorch execution interfaces. It detects CUDA/CPU execution devices, acts as the transformer sequence classifier loader, and instantiates the `LimeTextExplainer` class. If the model directory `./app/models/bias_model` is unpopulated, it triggers a mock data generator to prevent startup crashes.
...
sequenceDiagram
    ...
    participant ModelManager as PyTorch (BERT + LoRA)
...
```json
{
  ...
  "model_interpretation": "The text contains broad overgeneralizations ('always') combined with identity-based othering ('these people'). The phrasing is highly emotional rather than objective.",
  "attention_heatmap": [
    { "word": "these", "score": 0.082, "label": "highly_biased" },
    { "word": "people", "score": 0.054, "label": "highly_biased" },
    { "word": "are", "score": -0.01, "label": "neutral" },
    { "word": "always", "score": 0.031, "label": "suspicious" },
    { "word": "causing", "score": -0.01, "label": "neutral" },
    { "word": "problems", "score": 0.045, "label": "suspicious" }
  ],
  "sentence_vector": {
    "sentence": "These people are always causing problems because their ideology is dangerous.",
    "bias_type": "IDENTITY_BIAS"
  },
  "neutral_rewrite": "Some individuals frequently raise concerns due to their differing perspectives."
}
```
...
## 🤖 7. BERT + LoRA + LIME ML Pipeline

The heart of Unbiasly AI's local bias scoring consists of a PyTorch sequence classifier, Parameter-Efficient Fine-Tuning (PEFT), and Local Interpretable Model-agnostic Explanations (LIME).

### 1. Base Classifier
*   **Model**: `bert-base-uncased` (110 million parameters).
*   **Fine-Tuning Technique**: **LoRA (Low-Rank Adaptation)**. By keeping base model parameters frozen and introducing small rank-decomposition weight matrices ($r=8$) into the query and value projection layers of the self-attention modules, training VRAM requirement is reduced by 85%.
*   **Adapter Configs**:
...
1.  **Perturbation**: Input text is split into tokens. LIME creates a "neighborhood" of 5,000 text variations by randomly turning off (masking) different tokens.
2.  **Probability Mapping**: All perturbed texts are passed through `predict_proba` in a batch to get bias probability scores.
3.  **Weighted Linear Regression**: A localized linear regression model is fit to these predictions, weighting samples higher if they are more similar to the original text.
4.  **Weight Extraction**: The coefficients of this simple linear model represent how much each word contributed to the bias score. These are sent back as token scores for heatmap visualization.
...
services:
  ...
  backend:
    ...
    ports:
      - "8000:8000"
    volumes:
      - ./backend/app/models/bias_model:/app/app/models/bias_model
...
*   **Decoupled Heavy Model Weights**: The 1.5GB+ model weights folder is bind-mounted (`./backend/app/models/bias_model`) from the host rather than copied into the Docker image. This keeps container builds fast and lightweight.
